...
const Dashboard = () => {
  ...
  const [bookedSlots, setBookedSlots] = useState([]); // To track booked slots for the selected date
  ...
  const handleDateChange = (date) => {
    ...
    // Reset booked slots when the date is changed
    setBookedSlots([]); // Reset the booked slots on date change
    setCourts((prevCourts) =>
      prevCourts.map((court) => ({
        ...court,
        slots: court.slots.map((slot) => ({
          ...slot,
          available: true, // Enable all slots on date change
        })),
      }))
    );
  };
  ...
  const handleBooking = async (courtId, slot, date) => {
    try {
      ...
      // Disable the button immediately by updating the state
      setCourts((prevCourts) =>
        prevCourts.map((court) =>
          court._id === courtId
            ? {
      ...
      if (response.ok) {
        ...
        setBookedSlots((prevBookedSlots) => [
          ...prevBookedSlots,
          { courtId, slot, date: formattedDate },
        ]);

        setShowCourts(false); // Hide the courts after booking
      } else {
        ...
        {showCourts && (
          <div className="mt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {courts.map((court) => (
                <div key={court._id} className="bg-white p-4 border rounded shadow">
                  ...
                  <div className="mt-2">
                    {court.slots.map((slot) => (
                      <button
                        ...
                        disabled={!slot.available || bookedSlots.some(
                          (b) => b.courtId === court._id && b.slot === slot.slot && b.date === `${date.getDate().toString().padStart(2, '0')}-${(date.getMonth() + 1).toString().padStart(2, '0')}-${date.getFullYear()}`
                        )}
